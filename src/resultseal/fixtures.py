...
from resultseal.models import ClaimType, Contract, JsonValue, TruthState
...
class FixtureBundle:
    ...
    def loads(cls, text: str, limits: Limits) -> FixtureBundle:
        return _build(load_yaml(text.encode("utf-8"), limits))

    @classmethod
    def from_json_bytes(cls, data: bytes, limits: Limits) -> FixtureBundle:
        return _build(load_json(data, limits))
...
def _build(doc: object) -> FixtureBundle:
    ...
    contract = _embedded_contract(doc.get("contract"))
...
def _embedded_contract(value: object) -> Contract | None:
    if value is None:
        return None
...
def _expectation(value: object) -> Expectation:
    ...
    codes_raw = value.get("reason_codes") or []
