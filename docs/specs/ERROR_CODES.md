...
Use stable uppercase reason codes. Initial codes include `NO_DISPATCH`, `TRANSPORT_ERROR`, `PARSE_FAILED`, `EMPTY_WITHOUT_NOT_FOUND_SENTINEL`, `MISSING_REQUIRED_FIELD`, `STALE_OBSERVATION`, `SOURCE_MISMATCH`, `TARGET_MISMATCH`, `UNVERIFIED_EFFECT`, `SCHEMA_INVALID`, `UNSAFE_INPUT`, `LIMIT_EXCEEDED`, `UNSUPPORTED_CLAIM`, `CONTRACT_INVALID`, and `SEALED_WITH_REQUIRED_EVIDENCE`.
